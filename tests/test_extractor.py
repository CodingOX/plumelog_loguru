...
class TestFieldExtractor:
    ...
    def test_get_server_name_caching(self) -> None:
        """测试服务器名称缓存机制"""
        extractor = FieldExtractor()
    ...
    @patch("socket.socket")
    def test_get_server_name_primary_method(self, mock_socket: MagicMock) -> None:
        """测试主要的IP获取方法"""
        # 模拟socket连接成功
        mock_sock = MagicMock()
    ...
    @patch("socket.socket")
    @patch("socket.gethostname")
    def test_get_server_name_final_fallback(
        self, mock_gethostname: MagicMock, mock_socket: MagicMock
    ) -> None:
        """测试最终回退到127.0.0.1"""
        # 所有方法都失败
        mock_socket.side_effect = Exception("Socket error")
    ...
    @patch("socket.gethostname")
    def test_get_host_name_fallback(self, mock_gethostname: MagicMock) -> None:
        """测试主机名获取失败时的回退"""
        mock_gethostname.side_effect = Exception("Hostname error")
    ...
    def test_get_caller_info_no_frame(self) -> None:
        """测试无调用帧的情况"""
        extractor = FieldExtractor()
    ...
    def test_get_caller_info_with_method(self) -> None:
        """测试方法调用信息获取"""
        extractor = FieldExtractor()

        def test_function() -> CallerInfo:
    ...
    def test_get_next_seq_thread_safety(self) -> None:
        """测试序列号生成的线程安全性"""
        extractor = FieldExtractor()
        results: list[int] = []
